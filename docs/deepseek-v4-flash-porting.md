...
2. A DeepSeek expert packer for routed experts using 256 experts and 2048 intermediate size. For the antirez GGUF path this is implemented by `scripts/repack_deepseek_gguf_experts.py`.
3. New Metal dequant kernels for DeepSeek GGUF expert quantization: `IQ2_XXS` for gate/up and `Q2_K` for down.
...
1. Add a small Metal microbenchmark for one DeepSeek expert block.
2. Port or adapt the llama.cpp `IQ2_XXS` and `Q2_K` dequantized matvec logic.
3. Verify one expert forward: `down(silu(gate(x)) * up(x))`.
4. Compare output against llama.cpp/ggml for the same layer, expert, and input vector.
5. Only then wire routing/top-k and multi-expert combine into the full engine.
